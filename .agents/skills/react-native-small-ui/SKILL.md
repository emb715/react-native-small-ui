...
- This is a **utility toolkit**, NOT a component library
- Zero pre-built components (`<Box>`, `<HStack>`, `<Text>`, `<Card>` do not exist as exports)
...
// ❌ WRONG
function MyComponent() {
  const Card = Base.extend({ padding: 16 }); // same catastrophe as Rule 1
  return <Card />;
}
...
```tsx
// ❌ WRONG — imports theme hook from core
...
```tsx
// ❌ WRONG — no default theme exists
const theme = useTheme() as AppTheme;
theme.light.primary; // undefined — registerTheme was never called
...
// ❌ <Box />, <HStack />, <VStack />, <Center />, <Text /> — these are not exports
...
// ❌ theme.colors / theme.space / theme.palette — no default shape; these are your fields, if you define them
...
- `_<customPlatform>` — any key registered via `configure({ platforms: { tablet: () => bool } })`
- `_<customColorMode>` — any key registered via `configure({ colorModes: { highContrast: true } })`
...
```ts
const Extended = Base.extend({ padding: 16 });             // derive new component — MODULE LEVEL ONLY
const Compound = Base.withSlots({ Label: Text, Icon: View }); // dot-notation: <Compound.Label />
```

**createComponentGroup:**
```ts
// MODULE LEVEL ONLY — shared reactive context between siblings
const FormGroup = createComponentGroup({
  Root:  { Component: View,  style: { gap: 8 } },
  Label: { Component: Text,  style: { fontSize: 14 } },
  Input: { Component: TextInput },
});
// Usage: <FormGroup.Root><FormGroup.Label /><FormGroup.Input /></FormGroup.Root>
```
...
**Presets (spread into createComponent or StyleSheet.create):**
```ts
...
```
Available keys: `elevation.{none,xs,sm,md,lg,xl}` · `shadow.{none,soft,default,pronounced,inset}` · `inset.{none,safe,safeHorizontal,modal}` · `text.{fixed,crisp,accessible}` · `layout.{fill,center,row,rowBetween,column,absoluteFill}` · `border.{hairline,thin,medium,thick,pill}`
