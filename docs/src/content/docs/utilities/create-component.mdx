...
When slots need to reflect the parent's active **variant** (not just colorMode), chain `.withVariantContext()` after `.withSlots()`. See the [Button component](/components/button) for a complete example.
