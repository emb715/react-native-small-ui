...
The current implementation calls `createComponent` inside the render body, which creates a new component type on every render. This causes unnecessary remounts and defeats React reconciliation — the same problem described in the performance warning above. **Do not use `createThemedComponent` in production until this is resolved.** Use `createComponent` with theme values passed as props instead (shown below).
</Aside>

A wrapper that reads the active theme inside a render function and passes it into a style factory. Intended use case: component styles that depend on theme tokens and need to update when `setTheme()` is called.

**Signature:**

```ts
createThemedComponent(Component, (theme: unknown) => ComponentStyle)
```

The factory receives the raw theme value — cast it to your own type. It does **not** receive `_light`/`_dark` context, so conditional color mode styling must be done via `useColorModeValue` or `createComponent` props, not inside the factory.
...
type AppTheme = { primary: string; radius: number };

registerTheme({ primary: '#007AFF', radius: 8 });

// ✓ — use theme tokens directly, not _light/_dark
...
```tsx
// ✗ — _light/_dark inside the factory don't work as expected.
// The factory runs at render time before color mode is resolved.
const Bad = createThemedComponent(View, (theme) => ({
  _light: { backgroundColor: (theme as AppTheme).primary }, // misleading
  _dark:  { backgroundColor: '#000' },
}));

// ✓ — preferred pattern: createComponent at module scope, theme values as props
const Button = createComponent(TouchableOpacity, { borderRadius: 8 });

function ThemedButton() {
  const theme = useTheme() as AppTheme;
  return <Button backgroundColor={theme.primary} padding={12} />;
}
```

**Limitations vs `createComponent`:**
- No `.extend()`, `.withSlots()`, variants, or `ComponentConfig`
- Creates a new component type every render (implementation bug — see caution above)
- Returns a plain function component, not a `SmallUIComponent`
